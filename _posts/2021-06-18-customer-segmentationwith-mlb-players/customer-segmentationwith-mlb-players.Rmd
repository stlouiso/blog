...
A LOT of zero values. We need to figure out a good minimum for the number of games played a player needs to be included in our analysis. For this, instead of using analysis, I'm going to use business logic and require a player to have at least appeared in 100 games in the 2018 season.
...
The gap statistic suggests 1 cluster. Unfortunately the methods don't all agree which is usually the case when doing this analysis out in the wild. Fortunately we can visualize the clusters. Here we will visualize 3 and 4 clusters because 1 and 2 clusters aren't informative enough. 2 clusters would most likely segment good hitters from bad hitters and we want more unique groups than that. 
...
Cluster 1 = high walks, high strikeouts, low singles, low extras, high home runs

Cluster 2 = low walks, low strikeouts, high singles, high extras, high home runs

Cluster 3 = low walks, highish strikouts, low singles, low extras, low home runs

Cluster 4 = low walks, low strikeouts, high singles, avg extras, low home runs
...
All those hitters seem to be appropriately clustered except Carlos Santana. Carlos Santana walks a lot, but doesn't strikeout, and hits home runs at a slightly above average rate. He is a bit of a unique case so the extreme walk rate got him clustered to this group.

### Cluster 2 = All Around Good Hitters
They don't have a weak area, walk enough, hit a lot of singles and have power by hitting a lot of home runs. These are the well-rounded great hitters. We would expect the MVP winner Mookie Betts to be in this category. He might well be the most well-rounded hitter in the MLB with almost no weaknesses.
